    ...
    Axios.interceptors.request.use(
        (config) => {
            ...
            if (config.headers) {
                ...
                // 租户端token
                const tenantToken = tenantAdminInfo.getToken()
                if (tenantToken) (config.headers as anyObj).tetoken = tenantToken
            ...
            if (response.config.responseType == 'json') {
                if (response.data && response.data.code !== 1) {
                        ...
                        if (!window.tokenRefreshing) {
                            ...
                            return refreshToken()
                                ...
                                .catch((err) => {
                                    if (isAdminApp()) {
                                        ...
                                        tenantAdminInfo.removeToken()
                                        if (router.currentRoute.value.name != 'tenantLogin') {
                                            router.push({ name: 'tenantLogin' })
                                            return Promise.reject(err)
                                        } else {
                                            response.headers.batoken = ''
                                            window.requests.forEach((cb) => cb('', 'tenant-refresh'))
                                            window.requests = []
                                            return Axios(response.config)
                                        }
                                    } else {
                            ...
                            return new Promise((resolve) => {
                                ...
                                window.requests.push((token: string, type: string) => {
                                    ...
                                    } else if (type == 'tenant-refresh') {
                                        response.headers.tetoken = `${token}`
                                    } else {
                    ...
                    if (response.data.code == 303) {
                        ...
                        let routerPath = isAdminAppFlag ? adminBaseRoutePath : tenantBaseRoutePath
